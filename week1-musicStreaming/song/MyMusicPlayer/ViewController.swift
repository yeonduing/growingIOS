...
//        self.addViewsWithCode()
    ...
    func addViewsWithCode() {
        ...
        // addRepeatButton
//        self.addRepeatButton()
    }
    
    // RepeatButton layout 미완
    func addRepeatButton() {
        ...
        button.setTitle("Repeat O", for: UIControlState.normal)
        button.setTitle("Repeat X", for: UIControlState.selected)
        button.setTitleColor(.orange, for: UIControlState.normal)
        button.setTitleColor(.green, for: UIControlState.selected)
        
        button.addTarget(self, action: #selector(self.touchUpRepeatButton(_:)), for: UIControlEvents.touchUpInside)
        
        let centerX: NSLayoutConstraint
        centerX = button.centerXAnchor.constraint(equalTo: self.view.centerXAnchor)
        
        let left: NSLayoutConstraint
        left = timeLabel.leadingAnchor.constraint(equalTo: self.playPauseButton.trailingAnchor, constant: 8)
    ...
    // MARK: IBActions
    // MARK: touchUp action RepeatButton
    @IBAction func touchUpRepeatButton(_ sender: UIButton) {
        ...
        if sender.isSelected {
            ...
            self.repeatButton.layer.borderWidth = 1
            self.repeatButton.layer.cornerRadius = 5
            self.repeatButton.backgroundColor = .black
            self.repeatButton.setTitleColor(.white, for: .selected)
        } else {
            self.player?.numberOfLoops = .zero
            self.repeatButton.layer.borderWidth = 0
            self.repeatButton.backgroundColor = .none
        }
    }
